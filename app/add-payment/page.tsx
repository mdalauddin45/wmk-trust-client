...
import { Search, CircleDollarSign, User, MapPin, Wallet, Calendar, Info } from "lucide-react";
...
export default function AddPayment() {
  ...
  // মাস নির্বাচনের জন্য লিস্ট এবং বর্তমান মাস ডিফল্ট রাখা
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4">
      <div className="max-w-2xl mx-auto bg-white shadow-2xl rounded-3xl overflow-hidden border border-slate-100">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) => { setSelectedCenter(e.target.value); setSelectedMember(null); }}
              >
                <option value="">All Center</option>
                {[...Array(17)].map((_, i) => <option key={i} value={i + 1}>Center {i + 1}</option>)}
              </select>
            </div>
          ...
          {selectedMember && (
            <div className="bg-green-50 border border-green-200 p-4 rounded-2xl flex items-center justify-between gap-4 animate-in fade-in zoom-in duration-300">
              <div className="flex items-center gap-4">
                <div className="bg-green-600 p-3 rounded-full text-white"><User size={24} /></div>
                <div>
              ...
              <button type="button" onClick={fetchMemberHistory} className="p-3 bg-white border border-green-200 text-green-600 rounded-xl hover:bg-green-600 hover:text-white transition-all shadow-sm flex items-center gap-2 font-bold text-xs">
          ...
          <button disabled={loading} className="w-full bg-green-600 hover:bg-green-700 text-white font-extrabold py-4 rounded-2xl shadow-xl transition-all flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl">
            <div className="bg-slate-900 p-6 text-white flex justify-between items-center">
              <div><h2 className="text-xl font-black">পেমেন্ট রেকর্ড</h2><p className="text-xs opacity-60">{selectedMember?.name}</p></div>
              <button onClick={() => setShowHistory(false)} className="p-2 bg-white/10 rounded-full">✕</button>
            </div>
            <div className="p-6">
              <div className="max-h-[300px] overflow-y-auto space-y-3">
                {memberPayments.map((p, i) => (
                  <div key={i} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border">
                    <div><p className="font-bold text-slate-800">{p.month || 'N/A'}</p><p className="text-[10px] text-slate-400 font-bold uppercase">{new Date(p.date).toLocaleDateString()}</p></div>
                    <div className="text-right"><p className="font-black text-green-600">৳{p.amount}</p></div>
                  </div>
                ))}
              </div>
              <button onClick={() => setShowHistory(false)} className="w-full mt-6 bg-slate-100 py-3 rounded-xl font-bold">বন্ধ করুন</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-sm rounded-[3rem] p-8 text-center shadow-2xl border border-slate-100">
            <div className="relative w-24 h-24 mx-auto mb-6">
              ...
              <div className="relative bg-green-500 text-white w-24 h-24 rounded-full flex items-center justify-center shadow-lg"><CircleDollarSign size={48} /></div>
            </div>
            ...
            <div className="bg-slate-50 border rounded-2xl p-5 mb-8 text-left space-y-2">
              <div className="flex justify-between text-xs"><span className="text-slate-400 font-bold">MEMBER</span><span className="font-black">{lastPayment?.name}</span></div>
              <div className="flex justify-between text-xs"><span className="text-slate-400 font-bold">MONTH</span><span className="text-blue-600 font-black">{lastPayment?.month}</span></div>
              <div className="pt-2 border-t flex justify-between items-center"><span className="text-slate-400 font-bold text-[10px]">AMOUNT</span><span className="text-2xl font-black text-green-600">৳{lastPayment?.amount}</span></div>
            </div>
            <button onClick={() => setShowSuccess(false)} className="w-full bg-slate-900 text-white font-black py-4 rounded-2xl shadow-lg">ঠিক আছে</button>
          </div>
        </div>
      )}
    </div>
