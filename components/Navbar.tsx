...
export default function Navbar() {
  ...
  return (
    ...
    <div className={`fixed top-6 left-0 right-0 px-6 transition-all duration-500 ease-in-out ${
      isModalActive ? "z-0 opacity-0 pointer-events-none translate-y-[-20px]" : "z-[100] opacity-100 translate-y-0"
    }`}>
        ...
        <div className="px-6 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="hidden sm:block text-left leading-tight">
              <p className="text-sm font-black text-slate-900">WMK <span className="text-blue-600">TRUST</span></p>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter">V3.0 Smart Portal</p>
            </div>
          </Link>
             ...
             {navLinks.map((link) => (
               <Link 
                 ...
                 className={`flex items-center justify-between p-5 rounded-3xl font-black transition-all ${
                   pathname === link.path ? "bg-blue-600 text-white" : "bg-white/50 text-slate-700 border border-white/40 shadow-sm"
                 }`}
               >
                 <span className="flex items-center gap-4">{link.icon} {link.name}</span>
                 <ArrowRight size={16} className={pathname === link.path ? "text-white" : "text-slate-300"} />
               </Link>
             ))}
          </div>
        </div>
